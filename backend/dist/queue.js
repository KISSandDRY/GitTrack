...
exports.worker = exports.githubEventQueue = void 0;
...
exports.githubEventQueue = new bullmq_1.Queue('github-events', { connection });
exports.worker = new bullmq_1.Worker('github-events', async (job) => {
    ...
    try {
        ...
        if (!repoData || !sender)
            return { success: false, reason: 'No repo or sender data' };
        // 1. Ensure User exists
        let user = await prisma_1.prisma.user.findUnique({ where: { githubId: sender.id } });
        if (!user) {
        ...
        let repo = await prisma_1.prisma.repository.findUnique({ where: { githubId: repoData.id } });
        if (!repo) {
            repo = await prisma_1.prisma.repository.create({
                data: {
        ...
        if (event === 'push') {
            ...
            if (commits.length === 0)
                return { success: true, reason: 'No commits' };
            ...
            for (const commitData of commits) {
                ...
                await prisma_1.prisma.commit.create({
                    data: {
                        sha: commitData.id,
            ...
            const existingMetric = await prisma_1.prisma.dailyDeveloperMetric.findUnique({
                where: { date_userId_repoId: { date: today, userId: user.id, repoId: repo.id } }
            });
            if (existingMetric) {
                await prisma_1.prisma.dailyDeveloperMetric.update({
                ...
                await prisma_1.prisma.dailyDeveloperMetric.create({
                    data: {
                        date: today,
                        userId: user.id,
                        repoId: repo.id,
            ...
            if (payload.action === 'opened' || payload.action === 'closed') {
                await prisma_1.prisma.pullRequest.upsert({
